...
describe('Command Prompt Integration Tests', function () {
  ...
  describe('Configuration Integration', function () {
    ...
    it('should handle validation configuration', function () {
      ...
      assert.strictEqual(config.validate('hi'), 'Input must be at least 3 characters');
    ...
    it('should handle transformer configuration', function () {
      ...
      assert.strictEqual(config.transformer('hello', {}, { isFinal: true }), 'HELLO');
      assert.strictEqual(config.transformer('hello', {}, { isFinal: false }), 'hello');
    });
  });
  ...
  describe('Auto-completion Integration', function () {
    ...
    it('should handle function-based auto-completion', function () {
      ...
      assert.deepStrictEqual(config.autoCompletion('st'), ['start', 'stop', 'status']);
    ...
    it('should handle autocomplete customization options', function () {
      ...
      assert.deepStrictEqual(config.short('', ['a', 'b', 'c', 'd', 'e', 'f']), ['a', 'b', 'c', 'd', 'e']);
  ...
  describe('History Integration', function () {
    ...
    it('should work with FileBackedHistory', async function () {
      ...
      assert.deepStrictEqual(historyHandler.getAll(), ['file_cmd_1', 'file_cmd_2']);
  ...
  describe('Event Handlers Integration', function () {
    ...
    it('should handle onBeforeRewrite configuration', function () {
      ...
      assert.strictEqual(config.onBeforeRewrite('  HELLO   WORLD  '), 'hello world');
      assert.strictEqual(config.onBeforeRewrite('Test\t\tCommand'), 'test command');
    });
  ...
  describe('Theme and Display Integration', function () {
    it('should handle theme configuration', function () {
      ...
      assert.strictEqual(config.theme.style.message('Test', 'idle'), '[idle] Test');
      assert.strictEqual(config.theme.style.error('Error message'), '❌ Error message');
    });
  ...
  describe('Advanced Configuration Integration', function () {
    it('should handle complex configuration with all options', function () {
      ...
      const config = {
        ...
        autoCompletion: commands,
        autocompletePrompt: '📋 Available commands:',
        validate: (input) => input.length > 0 ? true : 'Command cannot be empty',
        transformer: (input, answers, flags) => flags.isFinal ? input.toUpperCase() : input,
