...
describe('FileBackedHistory', function () {
  ...
  describe('Configuration', function () {
    it('should use default configuration', function () {
      ...
      assert.strictEqual(history.config.fileName, 'inquirer-command-prompt-history.json');
    });
  ...
  describe('File Persistence', function () {
    it('should save history to file when save is enabled', async function () {
      ...
      assert.ok(await fsExtra.pathExists(FULL_HISTORY_PATH), 'History file should exist');
      
      const fileContent = await fsExtra.readJson(FULL_HISTORY_PATH);
      assert.deepStrictEqual(fileContent.history, ['saved_command1', 'saved_command2']);
    });
    ...
    it('should not save history to file when save is disabled', async function () {
      ...
      assert.ok(!(await fsExtra.pathExists(FULL_HISTORY_PATH)), 'History file should not exist');
    });
    ...
    it('should load existing history from file', async function () {
      ...
      assert.deepStrictEqual(history.history, ['existing_cmd1', 'existing_cmd2']);
      assert.strictEqual(history.historyIndex, 2);
    });
    ...
    it('should handle corrupted history file gracefully', async function () {
      ...
      sinon.assert.calledWithMatch(consoleLogStub, /Corrupted history file backed up to/);
    ...
    it('should handle directory creation errors gracefully', function () {
      const consoleErrorStub = sinon.stub(console, 'error');
      const ensureDirSyncStub = sinon.stub(fsExtra, 'ensureDirSync').throws(new Error('Permission denied'));
      ...
      sinon.assert.calledWithMatch(consoleErrorStub, /Could not create history directory/);
    ...
    it('should handle file write errors gracefully', async function () {
      const consoleErrorStub = sinon.stub(console, 'error');
      const writeFileSyncStub = sinon.stub(fsExtra, 'writeFileSync').throws(new Error('Disk full'));
      ...
      sinon.assert.calledWithMatch(consoleErrorStub, /Could not save history file/);
  ...
  describe('Manual save and load', function () {
    it('should allow manual save', async function () {
      ...
      assert.ok(!(await fsExtra.pathExists(FULL_HISTORY_PATH)), 'File should not exist before manual save');
      
      // Manually save
      history.save();
      
      // Now file should exist
      assert.ok(await fsExtra.pathExists(FULL_HISTORY_PATH), 'File should exist after manual save');
      
      const fileContent = await fsExtra.readJson(FULL_HISTORY_PATH);
      assert.deepStrictEqual(fileContent.history, ['manual_cmd1', 'manual_cmd2']);
    });
